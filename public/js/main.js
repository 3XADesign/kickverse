...
function guardarYAgregarAlCarrito() {
    ...
    if (!nombre || !dorsal) {
        showWarningModal('Por favor, completa todos los campos de personalización');
        return;
    }
...
function finalizarYAgregarAlCarrito() {
    ...
    if (formData.version === 'player') {
        ...
        if (nombreInput && dorsalInput) {
            ...
            if (!nombre || !dorsal) {
                showWarningModal('Por favor, completa el nombre y el dorsal');
                return;
            }
    ...
    if (!formData.liga || !formData.equipo || !formData.equipacion || !formData.talla) {
        showWarningModal('Faltan datos. Por favor, completa todos los pasos.');
        return;
    }
    ...
    showSuccessModal('Producto añadido al carrito correctamente', () => {
        // Abrir carrito
        openCart();
    });
}
...
function mostrarResumen() {
    ...
    if (!formData.liga || !formData.equipo || !formData.equipacion) {
        ...
        showErrorModal('Faltan datos del pedido. Por favor, completa todos los pasos.');
        return;
    }
    ...
    if (!stepContainer) {
        ...
        showErrorModal('No se encontró el contenedor del formulario');
        return;
    }
    ...
    try {
        ...
        showErrorModal('Error al mostrar el resumen: ' + error.message);
    }
}
...
function finalizarCompraCarrito() {
    if (cartItems.length === 0) {
        showWarningModal('El carrito está vacío');
        return;
    }
...
function agregarAlCarrito() {
    ...
    if (!version) {
        showWarningModal('Por favor selecciona una versión (FAN o PLAYER)');
        return;
    }

    if (!talla) {
        showWarningModal('Por favor selecciona una talla');
        return;
    }

    if (personalizar && (!nombre || !dorsal)) {
        showWarningModal('Por favor completa el nombre y dorsal');
        return;
    }
...
/**
 * Show notification from PHP session
 */
document.addEventListener('DOMContentLoaded', function() {
    // Check if there's a notification in the page data
    const notificationData = window.sessionNotification;
